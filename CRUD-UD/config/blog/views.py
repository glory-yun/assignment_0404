...
def create(request):
    ...
    new_blog.image = request.FILES.get('image') #imgae가 없을때 에러 방지를 위해 get을 사용
    new_blog.save()
...
def update(request, blog_id):
    ...
    old_blog.image = request.FILES.get('imgae')
    old_blog.save()
    return redirect('detail', old_blog.id) #post -> get 요청으로 바꿈 , 보낼때 post 요청 받는것은 get 요청임
